...
export async function generateCollegeInsights(
  ...
  try {
    ...
    const response = await client.chat.completions.create({
      model: MODEL,
      // Free-tier OpenRouter models frequently do internal chain-of-thought
      // reasoning before the final answer (see `reasoning`/`reasoning_details`
      // in the response) — that reasoning consumes tokens from the same
      // max_tokens budget. 600 was too tight and left `content: null` with
      // finish_reason "length" (all budget spent mid-thought). Verified
      // empirically that 2000 is enough headroom for reasoning + the JSON payload.
      max_tokens: 2000,
      messages: [
