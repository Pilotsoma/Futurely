...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      // Step 1: Log in to NextStep account
      ...
      if (showHac && hacUsername.trim() && hacPassword.trim() && hacUrl.trim()) {
        ...
        try {
          ...
          // Store only the URL — never the username or password
          localStorage.setItem('ns_hac_url', hacUrl.trim())
        } catch (hacErr) {
          // HAC connection failed — still navigate to dashboard but show warning
          setHacError(hacErr instanceof Error ? hacErr.message : 'School portal connection failed')
          // Wait 2 seconds so user can read the error before navigating
  ...
  const btnLabel = isLoading
    ? (step === 'connecting' ? 'Connecting to school portal...' : 'Logging in...')
    : 'Log In'
