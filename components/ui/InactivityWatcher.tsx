  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
        <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '1.2px', textTransform: 'uppercase', color: '#52698A' }}>
          Grade Viewer
        </span>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 5 }}>
          <span style={{ fontSize: 28, fontWeight: 800, color: '#2979FF', lineHeight: 1, letterSpacing: '-1px' }}>3.87</span>
          <span style={{ fontSize: 11, color: '#52698A', fontWeight: 600 }}>GPA</span>
        </div>
      </div>
      ...
      {subjects.map((s, i) => (
        ...
        >
          ...
          <span style={{ flex: 1, fontSize: 13, color: '#96AACC', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
            {s.name}
          </span>
          <span style={{ fontSize: 11, color: trendColor(s.trend), fontWeight: 700, minWidth: 28, textAlign: 'right' }}>
            {s.trend}{s.delta}
          </span>
          <span style={{ fontSize: 12, color: '#52698A' }}>{s.pct}%</span>
          <span style={{
  ...
  return (
    <div>
      ...
      <motion.div
        ...
        style={{ textAlign: 'center', margin: '20px 0 22px' }}
      >
        <div style={{ fontSize: 68, fontWeight: 800, lineHeight: 1, color: '#00E5FF', letterSpacing: '-3px' }}>
          {projectedGpa.toFixed(2)}
        </div>
        <div style={{ fontSize: 12, color: '#52698A', marginTop: 6, fontWeight: 600 }}>Projected GPA</div>
      </motion.div>
      ...
      >
        <div style={{
          ...
          <div style={{ fontSize: 11, color: '#52698A', marginBottom: 5, fontWeight: 600 }}>Current</div>
          <div style={{ fontSize: 22, fontWeight: 800, color: '#E8EEFF' }}>{currentGpa.toFixed(2)}</div>
        </div>

        <div style={{ color: '#00E5FF', fontSize: 20, fontWeight: 300, flexShrink: 0 }}>→</div>

        <div style={{
          flex: 1, background: 'rgba(0,229,255,0.08)', border: '1px solid rgba(0,229,255,0.28)',
          borderRadius: 12, padding: '12px 14px', textAlign: 'center',
        }}>
          <div style={{ fontSize: 11, color: '#00BCD4', marginBottom: 5, fontWeight: 600 }}>Projected</div>
          <div style={{ fontSize: 22, fontWeight: 800, color: '#00E5FF' }}>{projectedGpa.toFixed(2)}</div>
        </div>
      </motion.div>
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: '#52698A', marginBottom: 8, fontWeight: 600 }}>
          <span>Toward {targetGpa.toFixed(1)} target</span>
          <span style={{ color: '#00E5FF' }}>{progressPct.toFixed(0)}%</span>
        </div>
        <div style={{ background: '#1C2D47', borderRadius: 6, height: 8, overflow: 'hidden' }}>
          <motion.div
            initial={reduced ? false : { width: 0 }}
            animate={{ width: `${progressPct}%` }}
            transition={{ delay: 0.5, duration: 0.75, ease: [0.22, 1, 0.36, 1] }}
            style={{ height: '100%', borderRadius: 6, background: 'linear-gradient(90deg, #00BCD4, #00E5FF)' }}
          />
        </div>
      </motion.div>
    </div>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        ...
        <span style={{
          fontSize: 11, background: 'rgba(124,58,237,0.15)', color: '#A78BFA',
          border: '1px solid rgba(124,58,237,0.35)', borderRadius: 20, padding: '3px 10px', fontWeight: 700,
        }}>
          ✦ AI Sorted
        </span>
      </div>
        ...
        {assignments.map((a, i) => (
          <motion.div
            ...
            style={{
              ...
              borderLeft: `3px solid ${a.subjectColor}`,
              borderRadius: 10,
              padding: '11px 13px',
            }}
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: a.subjectColor, marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.6px' }}>
                  {a.subject}
                </div>
                <div style={{ fontSize: 13, color: '#E8EEFF', fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {a.title}
                </div>
                <div style={{ fontSize: 11, color: dueColor(a.status), marginTop: 4, fontWeight: 600 }}>
                  {a.due}
                </div>
              </div>
              <span style={{
  ...
  return (
    <div>
      ...
      <motion.div
        ...
        style={{ display: 'flex', justifyContent: 'center', margin: '18px 0 22px' }}
      >
        <div style={{ position: 'relative', width: 90, height: 90 }}>
          <svg width={90} height={90} style={{ transform: 'rotate(-90deg)' }} aria-hidden="true">
            <circle cx={45} cy={45} r={36} fill="none" stroke="#1C2D47" strokeWidth={8} />
            <motion.circle
          ...
          <div style={{
            ...
            <span style={{ fontSize: 22, fontWeight: 800, color: '#A855F7', lineHeight: 1 }}>68%</span>
            <span style={{ fontSize: 9, color: '#52698A', fontWeight: 600, marginTop: 3 }}>Complete</span>
          </div>
        </div>
      </motion.div>
        ...
        <div className="ns-dots">
          {SLIDES.map((s, i) => (
            <div
              ...
              className="ns-dot"
              style={{
                background: i === slideIndex ? s.color : '#273D5E',
